...
def load_model(model_path, device, model_type='mobilefacenet', embedding_size=512):
    if model_type == 'mobilefacenet':
        model = MobileFaceNet(embedding_size)
    elif model_type == 'ir_se':
        model = Backbone(num_layers=50, drop_ratio=0.6, mode='ir_se')
    elif model_type == 'ir':
        model = Backbone(num_layers=50, drop_ratio=0.6, mode='ir')
    else:
        raise ValueError("Unknown model type: choose 'mobilefacenet', 'ir', or 'ir_se'")
